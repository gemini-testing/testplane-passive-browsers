...
module.exports = (hermione, opts) => {
    ...
    hermione.on(hermione.events.BEFORE_FILE_READ, ({browser, testParser}) => {
        testParser.setController(config.commandName, {
            in: function(matcher) {
                const storage = this.suites ? suitesToRun : testsToRun;
                const browserId = this.browserId || browser; // hermione prior to version 7 does not set browserId property for suites
